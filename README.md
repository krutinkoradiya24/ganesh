...
require __DIR__ . '/vendor/autoload.php'; // Composer autoload (PHPMailer)

use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\Exception;

// === CONFIG - edit these ===
$to_email = 'krutinkoradiya24@gamil.com'; // the email where orders will be sent (change if typo)
$csv_file = __DIR__ . '/orders.csv';

// SMTP config - fill with your SMTP provider details
$smtpHost = 'smtp.gmail.com';      // e.g. smtp.gmail.com
$smtpPort = 587;                   // e.g. 587 for TLS, 465 for SSL
$smtpSecure = 'tls';               // 'tls' or 'ssl' or '' for none
$smtpUser = 'your-email@example.com'; // SMTP username (your email)
$smtpPass = 'your-smtp-password';     // SMTP password or app password

// From address used in email (should be same as smtpUser for many providers)
$fromEmail = $smtpUser;
$fromName  = 'GANESH MARKETING Orders';
...
function respond_html($html) {
    echo $html;
    exit;
}

// === read POST ===
$customerName = isset($_POST['customerName']) ? clean($_POST['customerName']) : '';
$customerPhone = isset($_POST['customerPhone']) ? clean($_POST['customerPhone']) : '';
$notes = isset($_POST['notes']) ? htmlspecialchars($_POST['notes']) : '';

$shopName = isset($_POST['shopName']) ? clean($_POST['shopName']) : 'GANESH MARKETING';
$shopContact = isset($_POST['shopContact']) ? clean($_POST['shopContact']) : '';
$shopAddress = isset($_POST['shopAddress']) ? clean($_POST['shopAddress']) : '';
...
if ($customerName === '' || $customerPhone === '') {
    respond_html("<p style='color:red'>Customer name and phone are required. <a href='index.html'>Go back</a></p>");
}
if (!is_array($itemNames) || count($itemNames) === 0) {
    respond_html("<p style='color:red'>Add at least one item. <a href='index.html'>Go back</a></p>");
}
...
for ($i = 0; $i < count($itemNames); $i++) {
    ...
    $iprice = isset($itemPrices[$i]) ? floatval($itemPrices[$i]) : 0;
    if ($iname === '' || $iqty <= 0) continue;
    $lineTotal = $iqty * $iprice;
    $grandTotal += $lineTotal;
    $lines[] = array('name'=>$iname,'qty'=>$iqty,'price'=>$iprice,'lineTotal'=>$lineTotal);
}

if (count($lines) === 0) {
    respond_html("<p style='color:red'>No valid items found. <a href='index.html'>Go back</a></p>");
}

// Build HTML email body
$subject = "New order — $shopName (" . date('Y-m-d H:i:s') . ")";
$body = "<html><body>";
$body .= "<h2>New Order — " . htmlspecialchars($shopName) . "</h2>";
$body .= "<p><strong>Customer:</strong> " . htmlspecialchars($customerName) . "<br>";
$body .= "<strong>Phone:</strong> " . htmlspecialchars($customerPhone) . "<br>";
$body .= "<strong>Notes:</strong> " . nl2br(htmlspecialchars($notes)) . "</p>";
...
foreach ($lines as $ln) {
    $body .= "<tr>";
    $body .= "<td>" . htmlspecialchars($ln['name']) . "</td>";
...
$body .= "<p><small>Shop: " . htmlspecialchars($shopName) . " | Contact: " . htmlspecialchars($shopContact) . " | Address: " . htmlspecialchars($shopAddress) . "</small></p>";
$body .= "</body></html>";

// Save CSV
$timestamp = date('Y-m-d H:i:s');
$items_json = json_encode($lines, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
$csv_row = array($timestamp, $customerName, $customerPhone, number_format($grandTotal,2), $items_json, $notes);
...
try {
    ...
    // optionally add reply-to with customer phone as text
    ...
    $mail->send();
    respond_html("<p class='success' style='background:#e8fff0;padding:12px;border-radius:8px'>Order sent successfully to <strong>" . htmlspecialchars($to_email) . "</strong>.<br><a href='index.html'>Place another order</a></p>");
} catch (Exception $e) {
    // mail failed but CSV saved
    $err = htmlspecialchars($mail->ErrorInfo);
    respond_html("<p class='error' style='background:#fff0f0;padding:12px;border-radius:8px'>Order saved, but email could not be sent. Error: {$err}. The order is stored in orders.csv. <a href='index.html'>Back</a></p>");
}
